...
//Api
import { addErrorEvent, callCreateUser } from '@/api/firebase';
import { isEmailInUseAction } from '@/server/actions/users';
...
export default function JoinForm({ orgNamesAndIds }: JoinFormProps) {
    ...
    const handleBlur = async (): Promise<void> => {
        validateEmail(email);
        if (!isInvalidEmail) {
            const emailInUse = await isEmailInUseAction(email);
